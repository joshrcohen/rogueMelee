...
static inline s32 ftYs_SpecialS_CheckButtonPressure(Fighter_GObj* gobj)
{
    ...
    s32 result = 0;
    if (fp->active_sticky.lstick.x < attrs->x98) {
        result = 1;
    }
...
void ftYs_SpecialS_8012F35C(Fighter_GObj* gobj)
{
...

    {
        ...
        if (fp2->active_sticky.lstick.x < attrs2->x98) {
            var_r5 = 1;
        }
        fp2->active_sticky.lstick.x = 0;
        if (var_r5 == 1) {
...
void ftYs_SpecialS_Enter(Fighter_GObj* gobj)
{
    ...
    fp->x74_self_accel.y = attributes->specials_start_gravity;
}
